...
function Dashboard({session,entries,goals,leads,proofItems,vaultItems,decisionItems,emailSummary,financialRows,reload,refreshEmail,setTab}:{session:Session,entries:DailyEntry[],goals:any[],leads:any[],proofItems:ProofItem[],vaultItems:VaultItem[],decisionItems:DecisionItem[],emailSummary:EmailSummary,financialRows:FinancialRow[],reload:()=>void,refreshEmail:()=>void,setTab:(t:string)=>void}) {
  ...
  const tomorrowAlert=async(a:SteveCommandAlert)=>{const d=new Date();d.setDate(d.getDate()+1);setBusy(a.key);const {error}=await supabase.from('goals').insert({user_id:session.user.id,title:a.title,next_action:`${a.action} · ${a.detail}`,deadline:d.toISOString().slice(0,10),status:'Inbox task'});setBusy(null);if(error){window.alert(error.message);return;}await saveAlertState(a,'done');await reload();};
  const matchingLead=(a:SteveCommandAlert)=>leads.find(l=>a.title.toLowerCase().includes((l.name||'').toLowerCase())||(a.detail||'').toLowerCase().includes((l.name||'').toLowerCase()));
  const callAlert=(a:SteveCommandAlert)=>{const l=matchingLead(a);if(l?.phone){window.location.href=`tel:${String(l.phone).replace(/\s/g,'')}`;}else{setTab(a.source==='Customers'?'Customers':'Sales');}};
  const emailAlert=(a:SteveCommandAlert)=>{const l=matchingLead(a);if(l?.email){window.location.href=`mailto:${l.email}?subject=${encodeURIComponent(a.title)}`;}else setTab('Email');};
  const actionLabel=(a:SteveCommandAlert)=>a.source==='Finance'?'Chase payment':a.source==='Email'?'Open email':a.source==='Sales'?'Open prospect':a.source==='Customers'?'Open customer':`Open ${a.source}`;
  const bucketFor=(a:SteveCommandAlert)=>a.priority>=90?'NOW':a.priority>=74?'TODAY':'WATCH';
  const nowAlerts=visibleAlerts.filter(a=>bucketFor(a)==='NOW'),todayAlerts=visibleAlerts.filter(a=>bucketFor(a)==='TODAY'),watchAlerts=visibleAlerts.filter(a=>bucketFor(a)==='WATCH');


  return <>
    <div className="card commandHero actionHero">
      <div><div className="kpiLabel">{greeting}, James · Steve Action OS</div><div className="heroText">{next?next.title:(latest?.mission||'The board is clear — choose the highest-value move.')}</div><p className="briefSummary">{next?next.action:'No unresolved high-priority commercial signal is currently visible.'}</p>
        {next&&<div className="actions heroActions"><button className="btn primary" disabled={busy===next.key} onClick={()=>doToday(next)}>Do this today</button>{(next.source==='Customers'||next.source==='Sales')&&<button className="btn" onClick={()=>callAlert(next)}>Call</button>}<button className="btn" onClick={()=>setTab(next.tab)}>{actionLabel(next)}</button><button className="btn" disabled={busy===next.key} onClick={()=>tomorrowAlert(next)}>Tomorrow</button></div>}
      </div><div className="commandScore"><span>Blueprint</span><strong>{blueprint.score}</strong><small>/100</small></div>
    </div>
    ...
    <div className="actionBuckets">
      <div className="actionBucket nowBucket"><div className="bucketHead"><span>NOW</span><strong>{nowAlerts.length}</strong><small>Highest commercial priority</small></div></div>
      <div className="actionBucket todayBucket"><div className="bucketHead"><span>TODAY</span><strong>{todayAlerts.length}</strong><small>Important before you finish</small></div></div>
      <div className="actionBucket watchBucket"><div className="bucketHead"><span>WATCH</span><strong>{watchAlerts.length}</strong><small>Keep on the radar</small></div></div>
    </div>

    <div className="grid commandMainGrid actionMainGrid">
      <div className="card"><div className="goalHeader"><div><div className="kpiLabel">STEVE ACTION QUEUE · {visibleAlerts.length} OPEN</div><h2>Work comes to you</h2></div><button className="btn" onClick={refreshEmail}>Refresh signals</button></div>
        <div className="commandAlerts">{visibleAlerts.slice(0,8).map((a,i)=><div className={`commandAlert actionAlert bucket${bucketFor(a)}`} key={a.key}><div className="alertRank">{i+1}</div><div className="alertBody"><div className="goalHeader"><div><span className={`bucketTag tag${bucketFor(a)}`}>{bucketFor(a)}</span><strong>{a.title}</strong></div><span className="sourcePill">{a.source}</span></div><p>{a.detail}</p><div className="actionReason"><span>{a.reason}</span><strong>{a.action}</strong></div>
          <div className="actions actionButtons"><button className="btn primary" disabled={busy===a.key} onClick={()=>doToday(a)}>Do Today</button>{(a.source==='Customers'||a.source==='Sales')&&<button className="btn" onClick={()=>callAlert(a)}>Call</button>}{(a.source==='Customers'||a.source==='Sales')&&<button className="btn" onClick={()=>emailAlert(a)}>Email</button>}<button className="btn" onClick={()=>setTab(a.tab)}>{actionLabel(a)}</button><button className="btn" disabled={busy===a.key} onClick={()=>tomorrowAlert(a)}>Tomorrow</button><button className="btn quietBtn" disabled={busy===a.key} onClick={()=>snooze(a)}>Snooze</button><button className="btn quietBtn" disabled={busy===a.key} onClick={()=>saveAlertState(a,'done')}>Done</button></div>
        </div></div>)}{!visibleAlerts.length&&<div className="emptyCommand"><strong>Nothing critical is shouting for attention.</strong><span>Keep Finance, Sales and Outlook current and Steve will surface new signals here.</span></div>}</div>
      </div>
...
function CustomerIntelligence({session,rows,leads,emailSummary,reload,setTab}:{session:Session,rows:FinancialRow[],leads:any[],emailSummary:EmailSummary,reload:()=>void,setTab:(t:string)=>void}){
  const [selected,setSelected]=useState(''); const [search,setSearch]=useState('');
  const money=(n:any)=>new Intl.NumberFormat('en-GB',{style:'currency',currency:'GBP',maximumFractionDigits:0}).format(Number(n)||0);
  const now=Date.now(), d28=28*86400000, d56=56*86400000; const by=new Map<string,any>();
  rows.forEach(r=>{const name=(r.customer||'').trim();if(!name)return;const key=name.toLowerCase();const x=by.get(key)||{name,current:0,prior:0,gp:0,due:0,overdue:0,last:null,orders:0};const t=r.row_date?new Date(r.row_date+'T12:00:00').getTime():0;if(t>=now-d28){x.current+=Number(r.sales)||0;x.gp+=Number(r.gross_profit)||((Number(r.sales)||0)-(Number(r.cost)||0));x.orders++;}else if(t>=now-d56)x.prior+=Number(r.sales)||0;x.due+=Number(r.amount_due)||0;if(Number(r.amount_due)>0&&r.due_date&&r.due_date<today)x.overdue+=Number(r.amount_due)||0;if(r.row_date&&(!x.last||r.row_date>x.last))x.last=r.row_date;by.set(key,x);});
  const customers=[...by.values()].map(c=>{const lead=leads.find(l=>(l.name||'').trim().toLowerCase()===c.name.toLowerCase());const email=lead?.email||'';const inbox=email?emailSummary.messages.filter(m=>(m.from?.emailAddress?.address||'').toLowerCase()===email.toLowerCase()):[];const decline=c.prior>0?(c.current-c.prior)/c.prior*100:null;const days=c.last?Math.floor((now-new Date(c.last+'T12:00:00').getTime())/86400000):999;let score=100;if(days>21)score-=30;else if(days>14)score-=15;if(decline!==null&&decline<-20)score-=25;else if(decline!==null&&decline<-5)score-=10;if(c.overdue>0)score-=20;else if(c.due>0)score-=5;if(c.current>c.prior*1.2&&c.current>0)score+=5;score=Math.max(0,Math.min(100,score));const status=score>=80?'Strong':score>=60?'Watch':score>=40?'At Risk':'Urgent';let recommendation='Maintain service and look for the next product opportunity.';if(c.overdue>0)recommendation=`Chase ${money(c.overdue)} overdue and protect the relationship.`;else if(days>21)recommendation='Reconnect now — this customer appears dormant.';else if(decline!==null&&decline<-20)recommendation='Call and understand the spend drop before it becomes churn.';else if(c.current>c.prior*1.2&&c.current>0)recommendation='Growing account — ask what else you can supply.';return {...c,lead,email,inbox,decline,days,score,status,recommendation};}).sort((a,b)=>a.score-b.score||b.current-a.current);
  const filtered=customers.filter(c=>c.name.toLowerCase().includes(search.toLowerCase())); const c=customers.find(x=>x.name===selected)||filtered[0]||customers[0];
  const createFollowUp=async(x:any)=>{if(x.lead)await supabase.from('sales_leads').update({follow_up_date:today,next_action:x.recommendation}).eq('id',x.lead.id);else await supabase.from('sales_leads').insert({user_id:session.user.id,name:x.name,stage:'Follow-up',follow_up_date:today,next_action:x.recommendation,source:'Customer Intelligence'});await reload();setTab('Sales');};
  const strong=customers.filter(c=>c.status==='Strong').length,watch=customers.filter(c=>c.status==='Watch').length,risk=customers.filter(c=>c.status==='At Risk'||c.status==='Urgent').length;
  return <><div className="card customerHero"><div className="kpiLabel">Steve · Customer Intelligence</div><div className="heroText">Customer 360</div><p className="briefSummary">Financial behaviour + Sales pipeline + Outlook signals in one commercial view.</p></div>
  <div className="grid cols4"><Kpi label="Customers analysed" value={customers.length}/><Kpi label="Strong" value={strong}/><Kpi label="Watch" value={watch}/><Kpi label="At risk / urgent" value={risk}/></div>
  {!customers.length?<div className="card"><h2>No customer data yet</h2><p className="muted">Import a customer-level Sage or Fresho CSV in Finance first.</p><button className="btn primary" onClick={()=>setTab('Finance')}>Open Finance</button></div>:<div className="customer360">
  <div className="card customerList"><div className="kpiLabel">CUSTOMER BOOK</div><h2>Who needs attention?</h2><input placeholder="Search customer…" value={search} onChange={e=>setSearch(e.target.value)}/><div className="list">{filtered.map(x=><button className={`customerRow ${c?.name===x.name?'active':''}`} key={x.name} onClick={()=>setSelected(x.name)}><span><strong>{x.name}</strong><small>{money(x.current)} · last {x.last||'—'}</small></span><span className={`healthPill health${x.status.replace(/\s/g,'')}`}>{x.score}</span></button>)}</div></div>
  {c&&<div className="customerDetail"><div className="card"><div className="goalHeader"><div><div className="kpiLabel">CUSTOMER HEALTH</div><h1>{c.name}</h1></div><div className={`healthBadge health${c.status.replace(/\s/g,'')}`}><strong>{c.score}</strong><span>{c.status}</span></div></div><div className="grid cols4 customerKpis"><Kpi label="Last 28 days" value={money(c.current)}/><Kpi label="Previous 28" value={money(c.prior)}/><Kpi label="GP" value={money(c.gp)}/><Kpi label="Outstanding" value={money(c.due)}/></div><div className="customerSignal"><div className="kpiLabel">STEVE RECOMMENDS</div><h2>{c.recommendation}</h2><p className="muted">{c.decline!==null?`Spend trend ${c.decline>=0?'+':''}${c.decline.toFixed(1)}%. `:''}{c.days<999?`${c.days} days since the latest imported sale. `:''}{c.overdue>0?`${money(c.overdue)} is past its imported due date.`:''}</p><button className="btn primary" onClick={()=>createFollowUp(c)}>Create sales follow-up</button></div></div>
  <div className="grid cols2"><div className="card"><h2>Sales relationship</h2>{c.lead?<><div className="listItem"><strong>{c.lead.stage||'Prospect'}</strong><br/><span className="muted">{c.lead.next_action||'No next action recorded'}</span></div><div className="listItem"><strong>{c.lead.contact_name||'No contact name'}</strong><br/><span className="muted">{c.lead.email||c.lead.phone||'No contact details'}</span></div><button className="btn" onClick={()=>setTab('Sales')}>Open Sales record</button></>:<><p className="muted">No matching Sales record yet.</p><button className="btn" onClick={()=>createFollowUp(c)}>Add to Sales</button></>}</div>
  <div className="card"><h2>Outlook signals</h2>{c.email?<><p className="muted small">{c.email}</p><div className="list">{c.inbox.slice(0,4).map((m:OutlookMessage)=><div className="listItem" key={m.id}><strong>{m.subject||'(No subject)'}</strong><br/><span className="muted">{m.receivedDateTime?new Date(m.receivedDateTime).toLocaleDateString('en-GB'):''}</span></div>)}{!c.inbox.length&&<div className="muted">No recent inbox messages matched.</div>}</div></>:<p className="muted">Add the customer email address in Sales to connect Outlook messages here.</p>}</div></div></div>}</div>}</>;
}
