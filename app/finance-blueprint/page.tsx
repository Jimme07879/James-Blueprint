...
function FinanceApp({ session }: { session: Session }) {
  ...
  const stockPurchaseTransactions = useMemo(() => transactions.filter(row => row.transaction_date.startsWith(selectedMonth) && row.category === "Stock Purchases"), [transactions, selectedMonth]);
  const stockPurchases = stockPurchaseTransactions.reduce((sum, row) => sum + Number(row.normalized_cost || 0), 0);
  const previousStockPurchases = previous ? transactions.filter(row => row.transaction_date.startsWith(previous.key) && row.category === "Stock Purchases").reduce((sum, row) => sum + Number(row.normalized_cost || 0), 0) : 0;
  const filteredTransactions = useMemo(() => {
    const query = search.trim().toLowerCase();
    const source = category === "Stock Purchases" ? stockPurchaseTransactions : monthTransactions;
    return source.filter(row => (category === "All" || row.category === category) && (!query || [row.nominal_code, row.nominal_name, row.reference, row.details, row.transaction_number].some(value => (value || "").toLowerCase().includes(query))));
  }, [monthTransactions, stockPurchaseTransactions, category, search]);
  const categories = [
